...
import { generateActivationToken, generateLicenseKey, hashSecret, isValidLicenseKey, licensePrefix, timingSafeEqualString } from './license-crypto';
...
export class LicensesService {
  ...
  private async findByLicenseKey(licenseKey: string): Promise<schema.License> {
    const rows = await this.db
      .select()
      .from(schema.licenses)
      .where(eq(schema.licenses.licenseHash, this.hash(licenseKey)))
      .limit(1);
