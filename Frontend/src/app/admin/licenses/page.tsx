...
export default function AdminLicensesPage() {
  ...
  useEffect(() => {
    refreshLicenses().catch(error => setError(error instanceof Error ? error.message : 'Failed to load licenses'));
  }, []);
      ...
      <form onSubmit={createLicense} className="rounded-lg border border-border bg-card p-5">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">The raw key is shown only once. Co-Op stores only the keyed hash.</p>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <div className="space-y-2">
            ...
            <Input id="seats" type="number" min={1} value={seats} onChange={event => setSeats(event.target.value)} required />
          </div>
          <div className="space-y-2">
            <Label htmlFor="maxDevices">Devices</Label>
            <Input id="maxDevices" type="number" min={1} value={maxDevices} onChange={event => setMaxDevices(event.target.value)} required />
          </div>
          <div className="space-y-2 md:col-span-2">
        ...
        {created && (
          <div className="mt-4 rounded-md border border-green-500/30 bg-green-500/10 p-4">
            <p className="text-sm font-medium text-green-700 dark:text-green-300">License generated</p>
            <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center">
              <code className="rounded bg-background px-3 py-2 text-sm">{created.licenseKey}</code>
            ...
            <tbody>
              {licenses.map(license => (
                <tr key={license.id} className="border-t border-border">
                  <td className="px-4 py-3">{license.customerEmail}</td>
              ...
              {licenses.length === 0 && (
                <tr>
                  <td className="px-4 py-8 text-center text-muted-foreground" colSpan={6}>No licenses generated yet.</td>
                </tr>
              )}
            </tbody>
          </table>
