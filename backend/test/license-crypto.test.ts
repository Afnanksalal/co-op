...
test('canonicalLicenseKey preserves lookup hashes for pasted keys', () => {
  const storedKey = 'COOP-ABCD-EFGH-JKLM-NPQR-STUV-WXYZ';
  const pastedKey = ' coop abcd-efgh jklm npqr stuv wxyz ';

  assert.equal(canonicalLicenseKey(pastedKey), storedKey);
  assert.equal(hashSecret(canonicalLicenseKey(pastedKey), 'pepper'), hashSecret(storedKey, 'pepper'));
});
